...
if __name__ == '__main__':
    ...
    rospy.init_node(config.dvs + '_simulator_node', anonymous=True)
    ...
    # Prepare publishers
    bridge = CvBridge()
    depthmap_pub = rospy.Publisher("/" + config.dvs + "/depthmap", Image, queue_size=0)
    image_pub = rospy.Publisher("/" + config.dvs + "/image_raw", Image, queue_size=0)
    pose_pub = rospy.Publisher("/" + config.dvs + "/pose", PoseStamped, queue_size=0)
    camera_info_pub = rospy.Publisher("/" + config.dvs + "/camera_info", CameraInfo, queue_size=0)
    event_pub = rospy.Publisher("/" + config.dvs + "/events", EventArray, queue_size=0)
    ...
    if write_to_bag:
        bag.write(topic='/' + config.dvs + '/pose', msg=make_pose_msg(positions[0], orientations[0], init_time), t=init_time)        
        
        bag.write(topic='/' + config.dvs + '/contrast_threshold', msg=Float32(C), t=init_time)
        bag.write(topic='/' + config.dvs + '/blur_size', msg=Int16(blur_size), t=init_time)
        ...
        bag.write(topic='/' + config.dvs + '/image_raw', msg=img_msg, t=init_time)
        ...
        bag.write(topic='/' + config.dvs + '/depthmap', msg=depth_msg, t=init_time)
    ...
    for frame_id in range(1, len(times)):
        ...
        if write_to_bag:
            bag.write(topic='/' + config.dvs + '/pose', msg=make_pose_msg(positions[frame_id], orientations[frame_id], timestamp), t=timestamp)
        ...
        if write_to_bag:
            bag.write(topic='/' + config.dvs + '/camera_info', msg=camera_info_msg, t=timestamp)
        ...
        if timestamp - last_pub_img_timestamp > delta_image or timestamp == init_time:
            ...
            if write_to_bag or image_pub.get_num_connections > 0:
                ...
                if write_to_bag:
                    bag.write(topic='/' + config.dvs + '/image_raw', msg=img_msg, t=timestamp)
            ...
            if write_to_bag or depthmap_pub.get_num_connections() > 0:
                ...
                if write_to_bag:
                    bag.write(topic='/' + config.dvs + '/depthmap', msg=depth_msg, t=timestamp)
        ...
        if timestamp - last_pub_event_timestamp > delta_event:
            ...
            if write_to_bag:
                bag.write(topic='/' + config.dvs + '/events', msg=event_array, t=timestamp)
